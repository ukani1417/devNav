...
describe('URLParser', () => {
  ...
  describe('construct', () => {
    ...
    it('should handle invalid parsed input', () => {
      ...
      expect(constructed.description).toContain('No base URL found');
    });
    ...
    it('should handle mixed dynamic and path tokens', () => {
      const parsed = parser.parse('dev session123 api user456 admin', mockConfig);
      const constructed = parser.construct(parsed, mockConfig);

      expect(constructed.isValid).toBe(true);
      expect(constructed.url).toBe('https://app.dev.com/session123/api/v1/user456/admin/dashboard');
    });
    ...
    it('should handle tokens with dashes', () => {
      ...
      expect(constructed.isValid).toBe(true);
      expect(constructed.url).toBe('https://staging-server.com/session123/admin/dashboard');
    });
  });
