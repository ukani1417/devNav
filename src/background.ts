...
class DevNavigatorExtension {
  ...
  private setupEventListeners(): void {
    ...
    chrome.omnibox.onInputChanged.addListener(
      (text: string, suggest: (suggestions: ChromeOmniboxSuggestion[]) => void) => {
   ...
   */
  private validateSuggestionContent(content: string, fallback: string = 'suggestion'): string {
  ...
  private async handleInputChanged(
    ...
    try {
      ...
      suggest(validSuggestions.length > 0 ? validSuggestions : []);
    } catch (error) {
   ...
   */
  private generatePartialSuggestions(text: string, parsed: ParsedInput): ChromeOmniboxSuggestion[] {
    const suggestions: ChromeOmniboxSuggestion[] = [];
   ...
   */
  private generateHelpSuggestions(_text: string, _parsed: ParsedInput): ChromeOmniboxSuggestion[] {
